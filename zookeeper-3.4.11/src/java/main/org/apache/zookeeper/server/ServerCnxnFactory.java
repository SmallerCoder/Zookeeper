...
public abstract class ServerCnxnFactory {
	...
	/**
	 * 创建一个ServerCnxnFactory,默认是NIOServerCnxnFactory
	 * @return
	 * @throws IOException
	 */
	static public ServerCnxnFactory createFactory() throws IOException {

		// 判断系统是否配置了zookeeper.serverCnxnFactory属性指定是NIOServerCnxnFactory还是NettyServerCnxnFactory
		String serverCnxnFactoryName = System.getProperty(ZOOKEEPER_SERVER_CNXN_FACTORY);
		// 如果没有配置，则默认使用NIOServerCnxnFactory
		...
		try {
			...
			// 返回ServerCnxnFactory对象
			return serverCnxnFactory;
		} catch (Exception e) {
			// 初始化ServerCnxnFactory失败，则抛出异常
	...
	static public ServerCnxnFactory createFactory(int clientPort, int maxClientCnxns)
			throws IOException {
		return createFactory(new InetSocketAddress(clientPort), maxClientCnxns);
	}

	static public ServerCnxnFactory createFactory(InetSocketAddress addr, int maxClientCnxns)
			throws IOException {
